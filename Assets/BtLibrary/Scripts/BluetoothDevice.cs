...
namespace TechTweaking.Bluetooth
{
	...
	public class BluetoothDevice 
	{
		...
		private int packetSize = 0;//Incase you have a reading_mode == LENGTH_PACKET
...
//			if(BtBridge.Instance.isPluginReady()){
...
//			}
		...
		//TODO This method shouldn't be exposed to Users
	   ...
	   private static void registerDevice (BluetoothDevice device, int id)
	   {
		...
		private void commitData ()
		{
			...
			if (isDeviceReady() && isNeedCommitData) {
				...
				if (isUsingUUID){
					javaBtConnection.Call ("setUUID", this.uuid);
				}
				if (this.willRead) 
					javaBtConnection.Call (ENABLE_READING, this.threadID);
                else javaBtConnection.Call(DISABLE_READING);
